...
import {
  ...
  drawDesertCentipedeHead, drawDesertCentipedeBody, drawCactus,
  ...
  drawFireSoldierAnt, drawFireWorkerAnt, drawFireQueenAnt,
  drawFireAntEgg, drawFireAntHole,
  drawScorpion, drawPyramid, drawTomb, drawMummifiedBeetle,
  soldierAntOffsetX,
} from './mobDrawing.js';
...
function drawTile(ctx, typeId, rarity, tier, cx, cy, discovered, killCount) {
  ...
  if (discovered) {
    ...
    switch (typeId) {
      ...
      case 'cactus': drawCactus(ctx, 0, 0, iconR * 1.1, true); break;
      case 'scorpion': {
      ...
      case 'tomb':    drawTomb(ctx, 0, 0, iconR * 1.3); break;
...
function measureTip(ctx, stats, hov, slots) {
  ...
  for (const row of STAT_ROWS_TIP) {
    ...
    if (row.key==='aggroRange') {
      ...
      else if (['cactus','ant_hole','beehive'].includes(hov.typeId)) v='Static (always hostile)';
