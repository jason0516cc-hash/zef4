import { petalInstances, damagePetal, hotbar, makePetal } from './petals.js';
...
export function spawnMobDrops(mob, isBoss = false) {
  ...
  switch (mob.typeId) {
    ...
    case 'desert_centipede_body':
      drop([
        { baseId: 'sand',           rateTable: DR_A },
        // Powder + Salt come later — add their drop slot once those petals exist
        { baseId: 'centipede_legs', rateTable: DR_C },
      ]);
      break;
    ...
    default:
      // Pyramid, Pharaoh's Tomb, Mummified Beetle, and other unhandled types — no drop yet
      break;
  }
...
export function updateCombat(dt) {
  ...
  for (const [mobId, poison] of poisonedMobs) {
    ...
    mob.hp -= tick; spawnDamage(mob.x, mob.y, tick, '#aa44ff', mob.radius, mob);
    if (mob.hp <= 0) {
      mob.dead = true;
      spawnMobDrops(mob, mob.isBoss); addXp(mobXpValue(rarityTier(mob.rarity), mob.isBoss ?? false));
  ...
  if (playerPoison.timer > 0 && !playerInvincible) {
    ...
    player.hp -= tick; triggerHurtFlash(player);    spawnDamage(player.x, player.y, tick, '#aa44ff', player.radius, player);
  ...
  for (const mob of mobs) {
    ...
    if (dist < minD && dist > 0.001) {
      ...
      if (!playerInvincible) {
        player.hp = Math.max(0, player.hp - damage); triggerHurtFlash(player);        if (player.hp <= 0) player.dead = true;
      }
      mob.alerted = true;
  ...
  for (let i = bossStingers.length - 1; i >= 0; i--) {
    ...
    if (dist2 < player.radius + s.radius && dist2 > 0.001) {
      ...
      if (!playerInvincible) {
        player.hp = Math.max(0, player.hp - dmg); triggerHurtFlash(player);        spawnDamage(player.x, player.y, Math.round(dmg), '#ff4444', player.radius, player);
        if (player.hp <= 0) player.dead = true;
      }
  ...
  for (const p of bossPeas) {
    ...
    if (pdist < player.radius + p.radius && pdist > 0.001) {
      ...
      if (!playerInvincible) {
        player.hp = Math.max(0, player.hp - dmg); triggerHurtFlash(player);        spawnDamage(player.x, player.y, Math.round(dmg), '#88ff44', player.radius, player);
        if (player.hp <= 0) player.dead = true;
      }
      // Bounce
